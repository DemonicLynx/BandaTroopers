	...
	for(var/i in GLOB.alive_human_list)
		...
		if(M.z && (M.z in z_levels) && !istype(M.loc, /turf/open/space))
			if(M.faction in FACTION_LIST_WY)
				num_pmcs++
			else if(M.faction == FACTION_MARINE || (M.faction in FACTION_LIST_UNSC && GLOB.RoleAuthority?.is_marine_equivalent_role(M.job, TRUE)))
				num_marines++
...
/datum/game_mode/proc/count_marines(list/z_levels = SSmapping.levels_by_any_trait(list(ZTRAIT_GROUND, ZTRAIT_RESERVED, ZTRAIT_MARINE_MAIN_SHIP)))
	...
	for(var/i in GLOB.alive_human_list)
		var/mob/M = i
		if(M.z && (M.z in z_levels) && !istype(M.loc, /turf/open/space))
			if(M.faction == FACTION_MARINE || (M.faction in FACTION_LIST_UNSC && GLOB.RoleAuthority?.is_marine_equivalent_role(M.job, TRUE)))
				num_marines++
...
/datum/game_mode/proc/count_per_faction(list/z_levels = SSmapping.levels_by_any_trait(list(ZTRAIT_GROUND, ZTRAIT_RESERVED, ZTRAIT_MARINE_MAIN_SHIP)))
	...
	for(var/mob/living/carbon/human/current_human as anything in GLOB.alive_human_list)
		...
		if(current_human.faction == FACTION_CLF)
			...
			num_headcount++
			continue
		if(current_human.faction == FACTION_MARINE || (current_human.faction in FACTION_LIST_UNSC && GLOB.RoleAuthority?.is_marine_equivalent_role(current_human.job, TRUE)))
			num_marines++
			num_headcount++
			continue
		num_headcount++
