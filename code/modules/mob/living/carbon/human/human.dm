...
/mob/living/carbon/human/get_status_tab_items()
	...
	if((faction == FACTION_MARINE || (faction in FACTION_LIST_UNSC && GLOB.RoleAuthority?.is_marine_equivalent_role(job, TRUE))) && !isnull(SSticker) && !isnull(SSticker.mode) && !isnull(SSticker.mode.active_lz) && !isnull(SSticker.mode.active_lz.loc) && !isnull(SSticker.mode.active_lz.loc.loc))
		. += "Primary LZ: [SSticker.mode.active_lz.loc.loc.name]"

	if((faction == FACTION_MARINE || (faction in FACTION_LIST_UNSC && GLOB.RoleAuthority?.is_marine_equivalent_role(job, TRUE))) && !isnull(SSticker) && !isnull(SSticker.mode))
