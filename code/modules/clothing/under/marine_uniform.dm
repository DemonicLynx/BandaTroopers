...
/obj/item/clothing/under/marine/veteran/UPP/Initialize(mapload, new_protection[] = list(MAP_ICE_COLONY = ICE_PLANET_MIN_COLD_PROT), override_icon_state[] = null)
	. = ..()
	// Kutjevo's UPP on-mob sprite uses the suffix naming convention.
	// The generic camouflage helper builds "d_upp_uniform", but this DMI
	// provides "upp_uniform_d" instead. Keep the held/inventory icon unchanged
	// and explicitly select the correct worn sprite for the base UPP uniform.
	if(SSmapping.configs[GROUND_MAP] && SSmapping.configs[GROUND_MAP].map_name == MAP_KUTJEVO)
		icon_state = "upp_uniform"
		item_state = "upp_uniform"
		worn_state = "upp_uniform_d"
