...
/obj/structure/machinery/computer/overwatch/proc/transfer_squad()
	...
	for(var/datum/squad/squad as anything in GLOB.RoleAuthority.squads)
		...
		// this Overwatch console exposes in its squad selector. This prevents
		// hidden/other-faction active squads from appearing here.
		if(squad.overwatch_officer || !overwatch_squad_faction_matches(squad) || squad.name == "Root")
			continue
