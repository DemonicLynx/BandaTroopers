...
/obj/vehicle/multitile/attackby(obj/item/O, mob/user)
	...
	if(istype(O, /obj/item/explosive/grenade))
		...
		if(nade.antigrief_protection && is_marine_equivalent_human(user) && explosive_antigrief_check(nade, user))
...
/obj/vehicle/multitile/proc/crew_mousedown(datum/source, atom/object, turf/location, control, params)
	...
	if(!hardpoint)
		if(!silent_hardpoint_warning)
			to_chat(source, SPAN_WARNING("Сначала выберите активный хардпоинт."))
		return
