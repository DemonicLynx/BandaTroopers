...
// Returns TRUE when a human belongs to the Marine-equivalent ground combat roles.
// This keeps HALO/UNSC Marines compatible with generic Marine infrastructure
// without treating all UNSC personnel (Navy/ONI/Police) as Marines.
/proc/is_marine_equivalent_human(mob/living/carbon/human/H, active_only = FALSE)
	if(!istype(H))
		return FALSE
	if(H.faction == FACTION_MARINE)
		return TRUE
	return H.faction in FACTION_LIST_UNSC && GLOB.RoleAuthority?.is_marine_equivalent_role(H.job, active_only)
