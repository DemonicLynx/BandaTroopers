...
/datum/authority/branch/role/proc/get_active_ship_primary_family_types(mode_name = GLOB.master_mode, datum/game_mode/mode_datum = SSticker.mode)
	...
	if(!islist(family_types) || !length(family_types))
		...
		family_types = family_types.Copy()

	// A map may explicitly allow more than one ship platoon family.
	// Keep those allowed families in RoleAuthority even when the map's default
	// platoon is different, so an alternate profile (for example HALO ODST on
	// Stalwart Frigate) still has its platoon/sections available to Overwatch.
	var/datum/map_config/map_cfg = SSmapping?.configs?[SHIP_MAP]
	if(map_cfg?.allowed_platoons)
		for(var/allowed_platoon_value in map_cfg.allowed_platoons)
			var/allowed_platoon_type = ispath(allowed_platoon_value) ? allowed_platoon_value : text2path(allowed_platoon_value)
			if(!allowed_platoon_type)
				continue
			var/datum/modular_ship_platoon_profile/allowed_profile = get_ship_platoon_profile_datum(allowed_platoon_type)
			var/list/allowed_family_types = allowed_profile ? allowed_profile.get_family_types() : list(allowed_platoon_type)
			if(!islist(allowed_family_types))
				allowed_family_types = list(allowed_platoon_type)
			for(var/family_type in allowed_family_types)
				add_unique_ship_platoon_value(family_types, family_type)
